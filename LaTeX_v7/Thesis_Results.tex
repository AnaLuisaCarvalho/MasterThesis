...
For the SM signal, the significance increases by aproximately $20\%$. Nonetheless, using the ATLAS default detector configuration the achieved significance is already above $5\sigma$: $S/\sqrt{B}=5.6\pm 0.6$. For the DM mediator, the significance also increases by approximately $20\%$. For the type II 2HDM with $m_H=900$ GeV, ...
...
The efficiency of the signal samples through the analysis is shown in figure \ref{fig:SSBvsGran} on the left for the SM signal, DM mediator and type II 2HDM. [DESCRIBE+QUANTIFY]

COMPARE THE DIFFERENT SIGNAL SAMPLES IN TERMS OF EFF 
...
\begin{figure}
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[trim={.6cm 0 0 0},clip,width=\linewidth]{./Figures/EffPlotGran.pdf}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[trim={0 0 .6cm 0},clip,width=\linewidth]{./Figures/SSBplotGran.pdf}
	\end{minipage}
	\label{fig:SSBvsGran}
	\caption{Signal efficiency as a function of the detector configuration for particle flow jets (squares) and pure calorimeter (HCAL) jets (triangles) (left). The error bars are drawn but are smaller than the markers. Significance $(S/\sqrt{B})$ as a function of the detector configuration for $\mathcal{L}=30~\text{ab}^{-1}$ (black) and $\mathcal{L}=3~\text{ab}^{-1}$ (blue) and for particle flow jets (squares) and pure calorimeter (HCAL) jets (triangles) for the SM signal sample (right).}
\end{figure}
...
\begin{figure}
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[trim={.6cm 0 0 0},clip,width=\linewidth]{./Figures/SSBplotGran.pdf}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[trim={0 0 .6cm 0},clip,width=\linewidth]{./Figures/SSBplotGran.pdf}
	...
	\caption{Significance $(S/\sqrt{B})$ as a function of the detector configuration for $\mathcal{L}=30~\text{ab}^{-1}$ (black) and $\mathcal{L}=3~\text{ab}^{-1}$ (blue) and for particle flow jets (squares) and pure calorimeter (HCAL) jets (triangles) for the DM mediator model (left) and for 2HDM type II (right).}
\end{figure}
